...
前端与后端各自部署在独立的 Cloudflare 服务中，通过环境变量共享配置。后端 Worker 需绑定 Cloudflare Queues、Prisma Accelerate、外部数据库及 Google APIs 的凭据。
...
| `TRANSCRIBE_QUEUE` (binding) | Worker | 是 | Cloudflare Queue 绑定（在 `wrangler.toml` 配置，而非普通 env 变量） |
...
Queue 消费者从 Cloudflare Queue 获取任务、执行长转写并写回数据库。确保 Worker API 在上传/提交转写时正确推送消息。
...
5. **异步转写**：任务应被推送到 Queue，消费者 Worker 日志显示处理并更新 `transcriptRun`。
...
- **Queue 监控**：在 Cloudflare 控制台查看消息堆积量，合理配置重试与 DLQ（死信队列）。
...
- 关联文件：`docs/cloudflare-deployment-steps.md`、`DEPLOYMENT.md`、`railway.toml`、`DATABASE_SETUP.md`
