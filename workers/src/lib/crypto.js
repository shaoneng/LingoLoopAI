...
async function deriveKey(password, salt, iterations, hash, length) {
    const keyMaterial = await crypto.subtle.importKey('raw', toArrayBuffer(encodeUtf8(password)), 'PBKDF2', false, ['deriveBits']);
    const derivedBits = await crypto.subtle.deriveBits({ name: 'PBKDF2', salt: toArrayBuffer(salt), iterations, hash }, keyMaterial, length * 8);
...
export function randomBase64Url(bytes = 48) {
