...
export async function createAccessToken(env, payload, expiresInSec = ACCESS_TOKEN_TTL_SEC) {
...
export async function createAuthSession({ prisma, userId, ttlDays = REFRESH_TOKEN_TTL_DAYS, }) {
...
export async function consumeRefreshToken({ prisma, token, }) {
    const hashed = await sha256(token);
...
export async function revokeSessionByToken({ prisma, token }) {
    const hashed = await sha256(token);
    await prisma.authSession.deleteMany({ where: { refreshToken: hashed } });
}
export async function createPasswordResetToken({ prisma, userId, ttlMinutes = PASSWORD_RESET_TOKEN_TTL_MIN, }) {
...
export async function consumePasswordResetToken({ prisma, token, }) {
    const tokenHash = await sha256(token);
    const now = new Date();
...
function refreshTokenExpiresAt(ttlDays) {
    const expires = new Date();
    expires.setUTCDate(expires.getUTCDate() + ttlDays);
    return expires;
}
function passwordResetTokenExpiresAt(ttlMin) {
    const expires = new Date();
